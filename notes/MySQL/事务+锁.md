...
> 关于read view创建的时机，
>
> 在REPEATABLE_READ级别下，事务开启的时候变创建了，这样才能保证在整个事务周期内，看到的数据都是一致的；
>
> 在READ_COMMITED级别下，每个statement都会重新创建，这样才能实现如果有其他事务更新提交，当前事务能看到
>
> 在READ_UNCOMMITED级别下，就不再创建了，每次都是读取最新版本的数据。（这样说的话，最新版本的数据并不是要提交后才生成的呢？关于这点，结合几种log buffer 详细了解一个更新落地的完整流程）
>
> 
...
越高越迷糊 网上说的各种各样的 只是想搞清楚mvcc的核心逻辑而已，莫非真要深入源码？？
