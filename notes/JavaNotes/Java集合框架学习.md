...
定位一个父节点，比如在堆中的第c排，第i个。（c和i均从1开始），推导其左节点下标和自己的下标关系。

![](https://cdn.sinaimg.cn.52ecy.cn/large/005BYqpgly1g50jjcx78qj30ht05l74r.jpg)

![](https://cdn.sinaimg.cn.52ecy.cn/large/005BYqpgly1g50jmfyyzaj30b6029aa0.jpg)

一个标准的堆，数字代表节点在堆中坐标，横坐标记为`c`,纵坐标记为`l`,前提准备，需要用到等比数列表达式和求和公式，这里不展开。

对于第c层，这一层的节点数量为$$2^{c-1}$$,

1到c层总节点数量为$$2^c-1$$,现在问题，求出$$i_左$$、$$i_右$$和$$i_父$$跟$$i$$的关系
...
1.每个节点要么是红的要么是黑的。ps:每个节点都有颜色，并且是红黑两者中的一个
