...
public class TransactionLevelSample {
     ...
     * <p>
     * 两个事务并发修改同一条数据，试验在不同隔离级别下。结果的区别
     * 两个事务跟是否一定需要两个线程？？？
     *
     * 所谓的事务本质是什么？反应在MySQL代码层面，是加锁了还是干什么了？
     *
     * JDBC每执行一条SQL都是在事务中吗？如果是，上层那些个事务传播是怎么控制的？
     * 针对第一个问题，先尝试从driver中能否得到答案
     *
     *
     * Connection级的事务隔离级别和MySQL服务器上设置的默认隔离级别什么关系
     * 我的理解事务级别是Connection级的。创建一个Connection本质上就是跟MySQL服务器建立一个socket连接。
     * 在此过程中，先从MySQL服务器上拿到默认的隔离级别设置给Connection,如果后续不再设置隔离级别，那么此次Connection
     * 的隔离级别就是MySQL服务器上设置的默认值
     *
